...
- An Exasol instance (in Docker)
- Trino configured with both connectors
...
```sql
-- 1. Iceberg side: create schema and table
CREATE SCHEMA IF NOT EXISTS iceberg.hr;

CREATE TABLE iceberg.hr.employee (
id INT,
username VARCHAR,
hashtype_col VARBINARY(32)
)
WITH (
format = 'PARQUET'
);

-- Insert sample data into Iceberg
...
-- 2. Exasol side: create schema and table
CREATE SCHEMA IF NOT EXISTS exasol.hr;

CREATE TABLE exasol.hr.employee (
id DECIMAL(18,0),
...
-- Insert sample data into Exasol
INSERT INTO exasol.hr.employee (id, username, hashtype_col, hashtype_col2)
VALUES
...
-- 4. Cross-catalog join query with readable hashtype columns
SELECT
    i.id,
    i.username AS iceberg_user,
    e.username AS exasol_user,

    -- Convert VARBINARY to readable hex string
...
-- 5. Cross-catalog query with trino varbinary column joining on exasol hashtype column
SELECT
    i.id,
    i.username AS iceberg_user,
    e.username AS exasol_user,
